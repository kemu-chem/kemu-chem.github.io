...
<figure>
  <img src="src/manual/misc/cell_sample_authentic_ref.png" alt="実際の蛍光顕微鏡画像(細胞クラスター)">
  <figcaption>サンプル画像。赤(細胞質など)・青(核)が密集したクラスターを作っており、まばらな緑/シアンのマーカー陽性細胞も見えます。<br>出典: Shirakawa, J.; Kulkarni, R. N.; Togashi, Y.; Basile, G.; Okuyama, T.; Inoue, R.; Fernandez, M.; Kyohara, M.; De Jesus, D. F.; Goto, N.; Zhang, W.; et al. <em>Cell Reports</em> <strong>2022</strong>, <em>41</em>, 111436.</figcaption>
</figure>
<p><a class="manual-locate-btn" href="src/manual/misc/cell_sample_authentic_ref.png" download>📥 サンプル画像を保存</a></p>
...
「Channel Parameters」のG・Bタブを開き、それぞれ「Otsu」にチェックを入れます(このサンプルではG≈107、B≈94)。次に「Min area」を5程度に、「Conditional Count」の「＋ Add condition」で条件を作り、1つ目をG、2つ目をBにします。なお、チャンネル別のしきい値は既定で「暗い背景に明るい信号」を正しく想定しているため、グレースケールの場合と違って「Invert」を触る必要はありません。</p>
<p>
...
<figure>
  <img src="src/manual/misc/authentic_cond_g_and_b.png" alt="G AND B の結果:19個のマーカー陽性細胞">
  <figcaption>「G AND B」の結果。密集したクラスターの中にあっても、マーカー陽性の細胞(19個)だけが正確に浮かび上がります。単純に「Gチャンネルだけ」を見た場合と同じ19個になり、マーカー信号のほとんどが核と重なっていることも確認できます。</figcaption>
</figure>

<p><strong>1-3. 次に大まかに:組織全体を一括カウントする(R AND B)</strong><br>
同じ「Conditional Count」の仕組みは、精密な絞り込みだけでなく、しきい値を調整すれば良好な一括カウントにも使えます。試しに別の条件行を作り、Rチャンネルのしきい値を44、Bチャンネルのしきい値を156に(どちらもOtsuではなく手動で)し、「Min area」を1、「Max area」を1000に設定してみてください。Rの低めのしきい値は「組織がある場所すべて」を大まかに拾うゲートの役割を果たします。</p>
<figure>
  <img src="src/manual/misc/authentic_cond_r_and_b.png" alt="R AND B の結果:およそ458個、組織全体の一括カウント">
  <figcaption>「R AND B」の結果(概算でおよそ458個 — この数値はシミュレーションによる近似で、実機での確認をお願いします)。密集クラスターの中の核が、かなり良好な精度で個別に検出されます。1-2のG AND Bとは目的が違い、こちらは「大まかな総数」を素早く把握するための設定です。</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>拡大例:接近した2つの核を含む領域が、1つの丸として自動検出されている。</figcaption>
</figure>
<figure>
...
R/G/Bを分けずに、画像全体をひとまとめ(グレースケール)で二値化するだけの、より単純な一括カウントも可能です。「Invert」を外し、「Threshold」を65程度、「Min area」を1、「Max area」を1000にすると、このサンプルでも300個台の細胞を一括検出できます(この数値も概算です)。仕組みの詳細は2で説明します。</p>
...
しきい値は、「この明るさより上を検出あり、下を検出なしとみなす」という境界線です。0〜255の値で指定します。「Otsu auto-threshold」は、画像の明るさの分布から、検出あり/なしがもっともくっきり分かれる境界線を統計的に自動計算する機能で、良い出発点になります。ただし常に最適とは限りません。1-3で「R・Bのしきい値をOtsuとは別の値(44・156)に手動で調整する」といった工夫をしたように、目的に応じて意図的にOtsuからずらすことで、狙った効果を得られる場合もあります。</p>
<figure>
  <img src="src/manual/misc/authentic_mask_r.png" alt="Rチャンネルのマスク(しきい値44)">
  <figcaption>Rチャンネル、しきい値44でのマスク(実際の「R mask」表示と同じ、赤=検出領域。1-3と同じしきい値)。組織がある場所の大部分が赤くなり、「ここに何かある」という大まかなゲートになっています。</figcaption>
</figure>
<figure>
  <img src="src/manual/misc/authentic_mask_b.png" alt="Bチャンネルのマスク(しきい値156)">
  <figcaption>Bチャンネル、しきい値156でのマスク(実際の「B mask」表示と同じ、青=検出領域。1-3と同じしきい値)。個々の核に近い、粒として分離した形になっています。</figcaption>
</figure>
...
<p>AND/OR/NOTは、2で見たような2値のマスク画像同士を、ピクセル単位で重ね合わせる操作です。1-2で使ったG・Bのしきい値(G≈107, B≈94)をそのまま使って、3つの演算がそれぞれ何をしているかを具体的に見ていきます。以下は、実際の「Mask」表示(検出領域を白で表示するモード)と同じ形式で、画像の一部を拡大したものです。</p>
<figure>
...
<figure>
  <img src="src/manual/misc/authentic_mask_crop_r.png" alt="この範囲のRマスク(赤、しきい値44)">
  <figcaption>この範囲のRマスク(実際の「R mask」表示と同じ、赤=検出領域。1-3・2-2と同じしきい値44)。大部分が赤くなり、個々の細胞の輪郭はほとんど区別できません。</figcaption>
</figure>
<figure>
...
「G AND B」は、Gのマスクで検出され、かつBのマスクでも検出されているピクセルだけを残します。1-2ですでに見た通り、結果は19個で、「Gだけ」を見た場合と同じ数になります。マーカー信号(G)のほとんどは、核(B)の中に収まっているということです。ANDは、2つの条件を両方満たす、絞り込まれた集合を数えたいときに向いています(共陽性・共局在)。</p>
<figure>
  <img src="src/manual/misc/authentic_mask_crop_and.png" alt="この範囲のAND(G AND B)マスク">
  <figcaption>「G AND B」のマスク。Gマスクとほぼ同じ形になります(Gの領域の大部分はBにも含まれるため)。</figcaption>
</figure>
<p>もう一つの例として、1-3・2-2で使ったR・Bのしきい値(R=44, B=156)で「R AND B」を見てみると、Bマスクに近い粒状の形になります。</p>
<figure>
  <img src="src/manual/misc/authentic_mask_crop_r_and_b.png" alt="この範囲のAND(R AND B、しきい値44・156)マスク">
  <figcaption>「R AND B」のマスク(R=44, B=156)。Rは範囲の大部分を占める広いチャンネルですが、ANDの結果はBマスクに近い粒状のまま残ります。ANDの形は、2つのうち狭い(厳しい)方のチャンネルに引っ張られる、という性質がここでも表れています。</figcaption>
</figure>
...
<figure>
  <img src="src/manual/misc/authentic_cond_g_or_b.png" alt="G OR B の結果:275個、ほぼB全体">
  <figcaption>画像全体で見た「G OR B」の結果(275個)。「B単体」で数えた場合(276個)とほぼ同じ数になります。Gはもともと少数でBの中にほぼ収まっているため、ORを取ってもBの範囲がそのまま結果を支配してしまうからです。</figcaption>
</figure>
<p>ORは、2つのチャンネルが互いに重ならない・別々の場所を示している場合に「どちらかで陽性なら数えたい(取りこぼしを防ぎたい)」という場面で威力を発揮します。今回のように一方がもう一方にほぼ含まれている場合は、広い方のチャンネル単体とほぼ同じ結果になり、あまり意味がありません。</p>
<p>これがさらに極端に出るのが「R OR B」です。Rはこの範囲のほぼ全域を占めているため、Bと組み合わせても新しい情報が加わらず、範囲全体がひとつながりの塊になってしまいます。</p>
<figure>
  <img src="src/manual/misc/authentic_mask_crop_r_or_b.png" alt="この範囲のOR(R OR B、しきい値44・156)マスク">
  <figcaption>「R OR B」のマスク(R=44, B=156)。個々の細胞どころか、範囲のほぼ全体が1つの塊としてつながってしまい、これでは何も数えられません。ORは、広い方のチャンネルが支配的になりすぎると、こういう結果になり得るという分かりやすい例です。</figcaption>
</figure>
...
<figure>
  <img src="src/manual/misc/authentic_cond_b_not_g.png" alt="B AND NOT G の結果:279個">
  <figcaption>画像全体で見た「B AND NOT G」の結果(279個)。「B単体」(276個)より減るどころか増えています。これは、上のマスク画像で見えるように、Gの領域がBの塊のちょうど内部にあった場合、そこを削ることで塊が2つに分裂し、個数としては逆に増えることがあるためです。NOTはピクセル単位の面積を削る操作であって、個数をそのまま引き算するわけではない、という点に注意してください。</figcaption>
</figure>
...
<figure>
  <img src="src/manual/misc/cell_sample_authentic_ref.png" alt="Real fluorescence microscopy image (cell clusters)">
  <figcaption>Sample image: red (e.g. cytoplasm) and blue (nuclei) form dense clusters, with a sparse green/cyan marker-positive population also visible.<br>Source: Shirakawa, J.; Kulkarni, R. N.; Togashi, Y.; Basile, G.; Okuyama, T.; Inoue, R.; Fernandez, M.; Kyohara, M.; De Jesus, D. F.; Goto, N.; Zhang, W.; et al. <em>Cell Reports</em> <strong>2022</strong>, <em>41</em>, 111436.</figcaption>
</figure>
<p><a class="manual-locate-btn" href="src/manual/misc/cell_sample_authentic_ref.png" download>📥 Save the sample image</a></p>
...
Open the G and B tabs under "Channel Parameters" and check "Otsu" on each (about 107 for G, 94 for B on this sample). Then raise "Min area" to about 5, click "＋ Add condition" under "Conditional Count", and set the first term to G and the second to B. Unlike the grayscale pipeline, per-channel thresholding already assumes "bright signal on a dark background" by default, so there's no need to touch "Invert" here.</p>
<p>
...
<figure>
  <img src="src/manual/misc/authentic_cond_g_and_b.png" alt="G AND B result: 19 marker-positive cells">
  <figcaption>Result of "G AND B". Even inside the dense clusters, only the marker-positive cells (19 of them) are precisely picked out. That matches the count from the G channel alone, confirming most of the marker signal genuinely overlaps real nuclei.</figcaption>
</figure>

<p><strong>1-3. Rough next: count the whole tissue in bulk (R AND B)</strong><br>
The same Conditional Count mechanism isn't just for precise filtering — tuned differently, it also gives a good bulk count. Try building another rule with the R channel threshold set to 44 and the B channel threshold set to 156 (both manually, not Otsu), with "Min area" at 1 and "Max area" at 1000. R's lower threshold acts as a rough gate for "anywhere tissue is present."</p>
<figure>
  <img src="src/manual/misc/authentic_cond_r_and_b.png" alt="R AND B result: roughly 458 cells, a bulk count of the whole tissue">
  <figcaption>Result of "R AND B" (roughly 458 by simulation — an approximation; please verify against the live tool). Nuclei throughout the dense clusters are individually detected with reasonably good accuracy. Unlike 1-2's G AND B, the goal here is a fast, rough total rather than a precise subset.</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>Close-up: a region spanning two adjacent nuclei, detected as a single circle.</figcaption>
</figure>
<figure>
...
You can also skip channels entirely and binarize the whole image at once (grayscale) for a simpler bulk count. Uncheck "Invert", set "Threshold" to about 65, "Min area" to 1, and "Max area" to 1000, and this sample detects a few hundred cells in one pass (also an approximation). Section 2 explains the mechanics behind this.</p>
...
A threshold is a cutoff line: "above this brightness counts as detected, below it doesn't," specified as a value from 0–255. "Otsu auto-threshold" statistically computes the cutoff that splits the image's brightness distribution most cleanly — a good starting point, but not always optimal. As in 1-3, where we deliberately set R and B to values (44 and 156) other than their Otsu thresholds, intentionally moving away from Otsu can get you a specific, useful effect.</p>
<figure>
  <img src="src/manual/misc/authentic_mask_r.png" alt="R channel mask (threshold 44)">
  <figcaption>The R channel mask at threshold 44 (same as the tool's actual "R mask" display — red = detected; same threshold as 1-3). Most of the tissue turns red: a rough gate for "something is here."</figcaption>
</figure>
<figure>
  <img src="src/manual/misc/authentic_mask_b.png" alt="B channel mask (threshold 156)">
  <figcaption>The B channel mask at threshold 156 (same as the tool's actual "B mask" display — blue = detected; same threshold as 1-3). This turns into distinct, grain-like blobs close to individual nuclei.</figcaption>
</figure>
...
<p>AND/OR/NOT overlay binary masks like the ones in section 2, pixel by pixel. Using the same G and B thresholds from 1-2 (G≈107, B≈94), here's exactly what each operation does. The crops below match the tool's actual "Mask" view (detected regions shown in white).</p>
<figure>
...
<figure>
  <img src="src/manual/misc/authentic_mask_crop_r.png" alt="R mask of this region (red, threshold 44)">
  <figcaption>The R mask for this region (same as the tool's actual "R mask" display — red = detected; threshold 44, same as 1-3 / 2-2). Most of the area turns red, with individual cell outlines barely distinguishable.</figcaption>
</figure>
<figure>
...
"G AND B" keeps a pixel only if it's detected in the G mask AND detected in the B mask. As we already saw in 1-2, the result is 19 — the same as "G alone." Most of the marker signal (G) falls inside a nucleus (B). AND is the right tool when you want a narrowed-down set that satisfies both conditions (co-positivity / co-localization).</p>
<figure>
  <img src="src/manual/misc/authentic_mask_crop_and.png" alt="AND (G AND B) mask of this region">
  <figcaption>The "G AND B" mask. Nearly identical to the G mask alone, since most of each G region is also inside B.</figcaption>
</figure>
<p>As a second example, using the R and B thresholds from 1-3 / 2-2 (R=44, B=156), "R AND B" comes out close to the B mask's grain-like shape.</p>
<figure>
  <img src="src/manual/misc/authentic_mask_crop_r_and_b.png" alt="AND (R AND B, thresholds 44/156) mask of this region">
  <figcaption>The "R AND B" mask (R=44, B=156). R is the broad channel here, covering most of the region, but the AND result still comes out close to B's grain-like shape. AND is pulled toward whichever of the two channels is narrower (stricter) — the same pattern as before.</figcaption>
</figure>
...
<figure>
  <img src="src/manual/misc/authentic_cond_g_or_b.png" alt="G OR B result: 275, essentially all of B">
  <figcaption>Result of "G OR B" across the whole image (275). Nearly identical to counting B alone (276). Since G is a small population almost entirely contained within B, OR-ing them just lets B's extent dominate the result.</figcaption>
</figure>
...
<p>"R OR B" takes this to an extreme. R already covers almost the entire region, so combining it with B adds no new information — the whole region merges into a single connected blob.</p>
<figure>
  <img src="src/manual/misc/authentic_mask_crop_r_or_b.png" alt="OR (R OR B, thresholds 44/156) mask of this region">
  <figcaption>The "R OR B" mask (R=44, B=156). Not just individual cells, but nearly the entire region merges into one connected blob — nothing left to count. This is a clear example of what happens when OR is dominated by an overly broad channel.</figcaption>
</figure>
...
<figure>
  <img src="src/manual/misc/authentic_cond_b_not_g.png" alt="B AND NOT G result: 279">
  <figcaption>Result of "B AND NOT G" across the whole image (279) — higher than B alone (276), not lower. As visible in the mask crop above, when a G region sits right in the middle of a B blob, cutting it out can split that blob into two separate pieces, increasing the count. NOT subtracts area pixel-by-pixel — it doesn't just subtract a number of objects.</figcaption>
</figure>
